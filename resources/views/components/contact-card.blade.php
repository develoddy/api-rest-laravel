    ...
    <div class="flex items-start">
        ...
        <div class="grow pl-3">
            ...
            <div class="text-sm mb-2">
                ...
                <x-icon name="chevron-right" class="h-3 w-3 inline-block" />
                @endif
                {{ $department }}
            </div>
            @if($mobile)
                <div class="text-sm mb-1">
                    <x-icon name="phone" class="h-4 w-4 inline-block mr-2 opacity-60" />
                    {{ $mobile }}
                </div>
            @endif
            @if($email)
                <div class="text-sm mb-1">
                    <x-icon name="chat-bubble-left-ellipsis" class="h-4 w-4 inline-block mr-2 opacity-60" />
                    {!! $email !!}
                </div>
            @endif
            @if($birthday)
                <div class="text-sm mb-1">
                    <x-icon name="calendar-days" class="h-4 w-4 inline-block mr-2 opacity-60" />
                    {{ $birthday }}
                </div>
            @endif
            {{ $slot }}
