...
<div class="relative bw-select bw-select-{{$input_name}} @if($add_clearing) mb-3 @endif"
    ...
    <div class="flex justify-between text-sm items-center rounded-md bg-white text-slate-600 border-2 border-slate-300/50
        ...
        <x-icon name="chevron-left" class="!-ml-3 hidden scroll-left" />
        <div class="text-left placeholder grow-0 text-blue-900/40 dark:text-slate-500">{{ $placeholder }}
            @if($required) <x-icon name="star" class="!text-red-400 !w-2 !h-2 mt-[-2px]" type="solid" /> @endif
        </div>
        ...
        <div class="whitespace-nowrap inline-flex">
            <x-icon name="chevron-right" class="scroll-right !-mr-1 hidden" />
            <x-icon name="x-circle" type="solid" class="reset w-6 h-6 fill-slate-300 hover:fill-slate-500 text-white dark:!text-dark-200 hidden dark:!fill-dark-700 dark:hover:!fill-dark-900" />
            <x-icon name="chevron-up-down" class="opacity-40 !ml-2" />
        </div>
    </div>
    <div class="w-full absolute z-30 rounded-br-lg rounded-bl-lg bg-white shadow-sm shadow-slate-400 border-2 
        ...
        @if($data !== 'manual')
            @foreach ($data as $item)
                <x-select-item label="{{ $item[$label_key] }}"
